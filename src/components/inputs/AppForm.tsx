...
const AppForm: React.FC<AppFormProps> = ({ fields, onSubmit, className, triggerSubmit, showSubmitButton, submitButtonLabel, initialData }) => {
	...
	// Expose handleSubmit to be triggered externally
	if (triggerSubmit) {
		...
		<div className="w-full h-full">
			<form onSubmit={handleSubmit} className={`${className || ""}flex flex-col gap-2`}>
				{fields.map((field) => (
					<div
						key={field.name}
						className={`${field.type === "textarea" || field.type === "multiple-select" || field.type === "file" ? "col-span-3" : ""}`}
					>
						{field.type === "select" ? (
							<AppDropdown
								label={field.label}
